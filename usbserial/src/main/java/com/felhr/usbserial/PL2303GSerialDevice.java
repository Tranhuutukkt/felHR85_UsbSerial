...
public class PL2303GSerialDevice extends UsbSerialDevice
{
    ...
    private boolean openPL2303G()
    {
        ...
        //Default Setup
        byte[] buf = new byte[1];
        if(setControlCommand(PL2303_REQTYPE_DEVICE2HOST_VENDOR, PL2303_VENDOR_READ_REQUEST, PL2303_READ_TYPE_HX_STATUS, 0, buf) < 0) // TODO. check TYPE_1 or TYPE_HN
            return false;

        if(setControlCommand(PL2303_REQTYPE_HOST2DEVICE_VENDOR, PL2303_VENDOR_READ_NREQUEST, PL2303_HXN_RESET_REG, PL2303_HXN_RESET_UPSTREAM_PIPE | PL2303_HXN_RESET_DOWNSTREAM_PIPE, null)< 0)
            return false;
